...
def _generate_markdown_report(report_data: dict) -> str:
    ...
    """
    lines = []
    lines.append("# 🛡️ ShieldMyRepo — Security Report Card\n")
    lines.append(f"**Repository**: `{report_data['repository']}`\n")
    lines.append(f"**Overall Grade**: **{report_data['grade']}** ({report_data['score']}/100)\n")
    lines.append("\n## Scanner Results\n")
    ...
    for scanner in report_data['scanners']:
        ...
        lines.append(f"| {scanner['name']} | {status_icon} {scanner['status']} | {len(scanner['findings'])} |")
